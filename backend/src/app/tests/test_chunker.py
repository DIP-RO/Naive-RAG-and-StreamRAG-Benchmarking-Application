...
async def test_chunker_emits_overlapping_chunks() -> None:
    ...
    if len(chunks) > 1:
        ...
        assert len(overlap) > 0, f"No overlap between chunks: {chunks[0].content[-40:]} || {chunks[1].content[:40]}"
...
async def test_chunker_single_sentence() -> None:
    ...
    chunks = chunker.chunk_document(document_id="doc1", title="Single", content="Just one sentence.")
    assert len(chunks) == 1
